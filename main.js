// main.js - VERSIÓN REFACTORIZADA (PLANIFICACIÓN ANUAL POR AÑO + VALIDACIÓN + MODELO UNIFICADO)
...
function validarPlanificacionCompleta() {
    ...
    if (!year || year === "") {
        alert("ERROR: Por favor, seleccioná el Año de la Planificación.");
        return false;
    }
    if (!nombre || nombre.trim() === "") {
        alert("ERROR: Por favor, ingresá tu Nombre completo.");
        return false;
    }
    if (!oficina || oficina === "") {
        alert("ERROR: Por favor, seleccioná tu Oficina.");
        return false;
    }
    ...
    if (preListAcm === '' || acmCaptacion === '' || captacionVenta === '' || listingPropio === '' || busquedas === '') {
        alert("ERROR: Completá todos los campos de 'Tus Efectividades (%)'. Si la efectividad es cero, ingresá 0.");
        return false;
    }
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (selectorAnio) {
            selectorAnio.value = currentYear;
        }

        // 2. Cargar los datos de la planificación para el año seleccionado (por defecto, el actual)
        await cargarDatosGuardados(user.uid);

        // 3. Agregar listener al selector de año para recargar el formulario
        if (selectorAnio) {
        ...
        // 4. LÓGICA DE ADMIN: Mostrar botón solo al jefe
...
if (btnObjetivo) {
    btnObjetivo.addEventListener('click', () => {
        ...
        if (gastoMensual > 0 && condicion > 0) {
            ...
            const formato = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(objetivoBruto);
            
            if (respuestaEl) {
                respuestaEl.textContent = `Tu Objetivo Anual Bruto: ${formato}`;
                respuestaEl.className = "mb-0 fw-bold text-success respuestaInputs"; 
            }
            const inputObj = document.querySelector('.objetivoAnual');
            if (inputObj) inputObj.value = objetivoBruto.toFixed(0);
        } else if (respuestaEl) {
            respuestaEl.textContent = "Por favor, completá ambos campos.";
...
function realizarCalculosYMostrar() {
    ...
    const elPre = document.querySelector('.respuestaCantidadPreListing');
    const elAcm = document.querySelector('.respuestaCantidadAcm');
    const elCapt = document.querySelector('.respuestaCantidadCaptaciones');
    const elVentTot = document.querySelector('.respuestaCantidadVentas');
    const elVentProp = document.querySelector('.respuestaCantidadVentasPropias');
    const elVentBusq = document.querySelector('.respuestaCantidadVentasBusquedas');
    const elComision = document.querySelector('.respuestaComisionPromedio');

    if (elPre) elPre.innerHTML = mostrarDatoHTML(datos.preListings);
    if (elAcm) elAcm.innerHTML = mostrarDatoHTML(datos.acms);
    if (elCapt) elCapt.innerHTML = mostrarDatoHTML(datos.captaciones);
    if (elVentTot) elVentTot.innerHTML = mostrarDatoHTML(datos.ventasTotales);
    if (elVentProp) elVentProp.innerHTML = mostrarDatoHTML(datos.ventasPropias);
    if (elVentBusq) elVentBusq.innerHTML = mostrarDatoHTML(datos.ventasBusq);
    ...
    if (elComision) {
        elComision.textContent = `Promedio de comisión por venta (3%): ${fmtComision}`;
    }
...
// ------------------------------------------------------------------
// 4. GUARDAR EN LA NUBE (CON AÑO, UID Y VALIDACIÓN)
// ------------------------------------------------------------------
...
if (btnGuardar) {
    btnGuardar.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!datosCalc) {
                alert("Error interno al calcular datos. Revisá los valores.");
                return;
            }

            const oficina = document.getElementById('miOficina').value;
            const year = document.getElementById('selector-plan-anio').value;
            const anioNumero = Number(year) || new Date().getFullYear();
            ...
            const datosAGuardar = {
                ...
                resultados: {
                    ventasTotales: Number(datosCalc.ventasTotales.toFixed(1)),
                    ventasPropias: Number(datosCalc.ventasPropias.toFixed(1)),
                    ventasBusquedas: Number(datosCalc.ventasBusq.toFixed(1)),
                    captaciones: Number(datosCalc.captaciones.toFixed(1)),
                    acm: Number(datosCalc.acms.toFixed(1)),
                    preListings: Number(datosCalc.preListings.toFixed(1))
                },
            ...
            await setDoc(
                doc(db, "planificaciones", `${currentUser.uid}_${year}`),
                datosAGuardar,
                { merge: true }
            );
...
if (btnPdf) {
    btnPdf.addEventListener('click', () => {
        ...
        const datos = obtenerDatosFormulario();
        if (!datos) {
            alert("Error interno al calcular datos para el PDF.");
            return;
        }
        ...
        // --- ESPACIO PARA CÓDIGO BASE64 DEL BACKGROUND ---
        // Si en algún momento querés usar una hoja membretada,
        // pegá aquí el string base64 de la imagen:
        ...
        const itemDato = (etiqueta, valor) => {
            ...
            docPdf.text(String(valor), colValor, y); 
            y += saltoLineaItem;
        };
...
// ------------------------------------------------------------------
// 6. CARGAR DATOS (POR AÑO: uid + year)
// ------------------------------------------------------------------
async function cargarDatosGuardados(uid) {
    try {
        ...
        // Limpiar campos antes de intentar cargar
        const inputGasto = document.querySelector('.gastoMensual');
        const inputCond = document.querySelector('.condicionDeAgente');
        const inputObj = document.querySelector('.objetivoAnual');
        const inputTicket = document.querySelector('.ticketPromedio');
        
        const inputPre = document.querySelector('.preListingAcm');
        const inputAcm = document.querySelector('.acmCaptacion');
        const inputCap = document.querySelector('.captacionVenta');
        const inputList = document.querySelector('.listingPropio');
        const inputBusq = document.querySelector('.busquedas');
        ...
        if (elPre) elPre.innerHTML = '-';
        if (elAcm) elAcm.innerHTML = '-';
        if (elCapt) elCapt.innerHTML = '-';
        if (elVentTot) elVentTot.innerHTML = '-';
        if (elVentProp) elVentProp.innerHTML = '-';
        if (elVentBusq) elVentBusq.innerHTML = '-';
        if (elComision) elComision.textContent = '';
        ...
        if (docSnap.exists()) {
            ...
            if (data.nombreAgente) {
                const elNom = document.getElementById('miNombre');
                if (elNom) elNom.value = data.nombreAgente;
            }
            if (data.oficina) {
                const elOf = document.getElementById('miOficina');
                if (elOf) elOf.value = data.oficina;
            }
            
            if (inputGasto && data.gastoMensual) inputGasto.value = data.gastoMensual;
            if (inputCond && data.condicionAgente) inputCond.value = data.condicionAgente;
            if (inputObj && data.objetivoAnual) inputObj.value = data.objetivoAnual;
            if (inputTicket && data.ticketPromedio) inputTicket.value = data.ticketPromedio;
            
            if (data.efectividades) {
                if (inputPre) inputPre.value = data.efectividades.preListAcm ?? "";
                if (inputAcm) inputAcm.value = data.efectividades.acmCapt ?? "";
                if (inputCap) inputCap.value = data.efectividades.captVenta ?? "";
                if (inputList) inputList.value = data.efectividades.listingPropio ?? "";
                if (inputBusq) inputBusq.value = data.efectividades.busquedas ?? "";
            }
            
            // Recalcular y mostrar resultados después de cargar todos los datos
